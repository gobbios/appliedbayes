...
- study species: a hypothetical primate species

  - more than one offspring per year is rare

- we assume the relationship is linear[^fgfgfg]
::: 



[^fgfgfg]: spoiler: it isn't

## a funky example - the data
...
```{r}
knitr::kable(data.frame(age = predvals, offspring = y))
```

. . .

::: {style="font-size: 0.8em" }
- small sample you say?[^iufdgdg]
:::


[^iufdgdg]: the minimum sample size in the Bayesian world is ~~1~~0

<!-- , oh wait no it's -->

## prior predictive simulation

```{r flat_prior, fig.width=4, fig.height=2.3, fig.align='center', out.width="75%", fig.dpi=600, include=FALSE, fig.path="figures/"}
set.seed(1)
par(yaxs = "i", xaxs = "i")
plot(0, 0, xlim = c(-20, 20), ylim = c(0, 0.025), type = "n", xlab = "parameter value (intercept or slope)", ylab = "density", cex.axis = 0.8, axes = FALSE)
axis(1)
# segments(-50, 0, 50, 0)
xseq <- seq(-20, 20, by = 0.1)
points(xseq, dnorm(xseq, 0, 20), type = "l", lwd = 5)
```

::: {.fragment fragment-index=1 .incremental style="font-size: 0.8em"}
- emulate a frequentist model

- equivalent to pretending we have no clue what values parameters have, with uninformative (*flat*) prior distributions
  - intercept: `normal(mean = 0, sd = 20)`
  - slope: `normal(mean = 0, sd = 20)`
:::


::: {.fragment fragment-index=2 .incremental style="font-size: 0.8em"}

![](figures/flat_prior-1.png){.fragment width=50% fig-align="center"}
...
::: {.column width="50%" .incremental .fragment style="font-size: 0.8em"}

- plausible?

- many predictions are so outrageous, they didn't even show in the plotting range

- (doesn't require observed data)
:::
:::
...
- slope:
  - must be positive
  - shouldn't allow implausibly large offspring numbers
...
```{r betterpriors1, fig.retina=2, fig.width=4, fig.height=2.3, fig.align='center', out.width="90%", fig.dpi=600, include = FALSE,  fig.path="figures/"}
set.seed(1)
par(yaxs = "i", xaxs = "i")
plot(0, 0, xlim = c(-20, 20), ylim = c(0, 0.21), type = "n", xlab = "parameter value (intercept)", ylab = "density", cex.axis = 0.8, axes = FALSE)
axis(1)
# segments(-50, 0, 50, 0)
xseq <- seq(-20, 20, by = 0.1)
points(xseq, dnorm(xseq, -1.5, 2), type = "l", lwd = 2)
```


```{r betterpriors2, fig.retina=2, fig.width=4, fig.height=2.3, fig.align='center', out.width="90%", fig.dpi=600, include = FALSE, fig.path="figures/"}
set.seed(1)
par(yaxs = "i", xaxs = "i")
plot(0, 0, xlim = c(-20, 20), ylim = c(0, 0.81), type = "n", xlab = "parameter value (slope)", ylab = "density", cex.axis = 0.8, axes = FALSE)
axis(1)
# segments(-50, 0, 50, 0)
xseq <- seq(-20, 20, by = 0.1)
points(xseq, dnorm(xseq, 0.5, 0.5), type = "l", lwd = 2)

```

:::{.columns}

::: {.column width="50%" .fragment .incremental style="font-size: 0.8em"}

  - intercept:
  
  - `normal(mean = -1.5, sd = 2)`

![](figures/betterpriors1-1.png){.fragment width=100% fig-align="center"}
...
::: {.column width="50%" .fragment .incremental style="font-size: 0.8em"}
  ...
  - `normal(mean = 0.5, sd = 0.5)`

![](figures/betterpriors2-1.png){.fragment width=100% fig-align="center"}
...
- we can also use our data (for the predictor variable), e.g.:
:::
...
```{r prior_check_2_withdata_a, fig.width=4, fig.height=2.3, fig.align='center', out.width="65%", fig.dpi=600, fig.path="figures/"}
set.seed(1)
ymax <- sapply(seq_len(1000), \(x) max(rpois(nrow(xdata), exp(rnorm(1, -1.5, 2) + rnorm(1, 0.5, 0.5) * xdata$age))))
hist(ymax, breaks = seq(-0.0000001, max(ymax) + 20.0000001, by = 2), xlim = c(0, 60), yaxs = "i", ylim = c(0, 500),
     xlab = "max number of offspring", las = 1, main = "", ylab = "frequency")
box(bty = "l")
```
...
::: {.column width="50%"}
::: {style="font-size: 0.8em" .fragment fragment-index=4}
proportion without any offspring?
:::
::: {.fragment fragment-index=5}

```{r prior_check_2_withdata_b, fig.width=4, fig.height=2.3, fig.align='center', out.width="65%", fig.dpi=600, fig.path="figures/"}
set.seed(1)
yprop0 <- sapply(seq_len(1000), \(x) mean(rpois(nrow(xdata), exp(rnorm(1, -1.5, 2) + rnorm(1, 0.5, 0.5) * xdata$age)) == 0))
barplot(table(yprop0), xlab = "proportion individuals without offspring", ylab = "frequency")
```
:::
:::
:::
...
- there is no absolute right or wrong: it’s about general plausibility of our prior choice
:::
